...
function GetDate(str) {
    ...
    for(i; i < monthValue.length; i++) {
        var j = 0;
        for(j; j < monthValue[i].length; j++) if (monthValue[i][j] == listOfDate[1]) break;
...
function FillBlocks() {
    ...
    var a = IntegerDivision((14 - current_month + 1), 12);
    var y = current_year - a;
    var m = current_month + 1 + 12 * a - 2;
    var first_date_in_month = (7000 + (1 + y + IntegerDivision(y, 4) - IntegerDivision(y, 100) + IntegerDivision(y, 400) +
        IntegerDivision(31 * m, 12))) % 7;
...
function DisplayBigForm(thiss) {
    ...
    if (flagaddbigevent == false) {
        ...
        if (pos.top + 329 + $(thiss).height() - 15 > page_heigth) {
            pos.top = pos.top - 329 + $(thiss).height();
...
function SearchAction(source) {
    ...
    for(var i = 0; i < localStorage.length; i++) {
        ...
        if (even.year > current_date.getFullYear() 
                        || (even.year == current_date.getFullYear() && even.month > current_date.getMonth()) 
                        || (even.year == current_date.getFullYear() && even.month == current_date.getMonth() && even.date >= current_date.getDate())) {
...
function SortStorage(source) {
    ...
    massForSearch.sort(function (a,b) { return  - a.count + b.count });
...
function GetFormForSearch(keys) {
        ...
        for ( var i = 0; i < keys.length; i++ ) {
            ...
            $('.search').append('<div class="search_block"><div class="search_block_text-title">'+ elem.event + '</div><div class="search_block_text-date">' + elem.date + ' ' + monthsPOD[elem.month] + bufYear + '</div></div><div class="line"></div>');
        }
